...
Fraction Fraction::operator-(int n) const {
    ...
    result.numerator -= n * result.denominator;
    result.reduce();
    return result;
}

Fraction operator-(int n, const Fraction& f) {
    Fraction result(n, 1);  // Create a fraction representing the integer n
    result -= f;  // Use the already-implemented operator-= from the Fraction class
    return result;
}
